...
async function run() {
  ...
  server.listen(5002, async () => {
    try {
      ...
      const joinRes = await fetch('http://localhost:5002/api/join-room', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ roomId, username: 'Pad1', role: 'Pad Holder', teamName: 'CSK' })
      }).then(r => r.json());
      console.log('Padholder joined:', joinRes);

      const roomRes = await fetch(`http://localhost:5002/api/room/${roomId}`).then(r => r.json());
      console.log('Room GET succeeded!', roomRes);
    } catch(err) {
